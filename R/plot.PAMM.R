  ...
  function (x, graphtype = "both", nbgroup, nbrepl, fun3D = "wireframe",...)
  {
    ...
    if (graphtype == "group") {
      ...
      for ( i in 1:4) {
        ...
        lines(x$nb.ID[x$nb.repl == nbrepl], x[x$nb.repl == nbrepl, paste("CIup.",ys[i], sep="")], lty = 2)
        lines(x$nb.ID[x$nb.repl == nbrepl], x[x$nb.repl == nbrepl, paste("CIlow.",ys[i], sep="")], lty = 2)
      }
    ...
    if (graphtype == "repl") {
      ...
      for ( i in 1:4) {
        plot(x$nb.repl[x$nb.ID == nbgroup], x[x$nb.ID == nbgroup,xs[i]], type = "l", 
             xlab = "", ylab = "", ylim = c(0, 1),bty="L")
        if (i<=2) abline(h = 0.05)
        lines(x$nb.repl[x$nb.ID == nbgroup], x[x$nb.ID == nbgroup, paste("CIup.",ys[i], sep="")], lty = 2)
        lines(x$nb.repl[x$nb.ID == nbgroup], x[x$nb.ID == nbgroup, paste("CIlow.",ys[i], sep="")], lty = 2)
      }
    ...
    if (graphtype == "both") {
      if (fun3D == "wireframe") {
        ...
        for (i in 1:4) {
          p <- wireframe(as.formula(paste(xs[i]," ~ nb.ID + nb.repl")), x, colorkey = FALSE, 
                         drape = TRUE, scales = list(arrows = FALSE, distance = c(2,2,2)), xlab = "Group", 
                         ylab = "Repl", main = mains[i],
